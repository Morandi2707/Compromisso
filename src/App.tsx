...
function App() {
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot: QuerySnapshot) => {
        const guestsData = snapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
          confirmationDate: new Date(doc.data().confirmationDate.seconds * 1000),
        })) as Guest[];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const guestRef = doc(collection(db, 'Convidados'));
  ...
  const handleDeleteGuest = async (guestId: string) => {
    try {
      await deleteDoc(doc(db, 'Convidados', guestId));
    } catch (error) {
  ...
  const backgroundStyle = {
    backgroundImage: `url('https://drive.google.com/file/d/1lx94uoie-uCIADQupXnaBsN8S0R_MdM4/view?usp=sharing')`, 
